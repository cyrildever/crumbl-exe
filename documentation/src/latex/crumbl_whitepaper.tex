...
\begin{algorithm}
    ...
    use (\ref{eq:slice}) on $\mathcal{S} := \{ \sigma_t \}$ to recover $d$: \begin{small} $$
        d \gets \mu^{-1}(\sigma_0) \concat \mu^{-1}(\sigma_1) \concat \dots \concat \mu^{-1}(\sigma_{t-1})
    $$ \end{small} \\
...
Let $\mathfrak{F}()$ be an implementation of an almost Format-Preserving Encryption scheme based on a Feistel\cite{feistel:hf} cipher\footnote{With 
our implementation described in \cite{Feistel:cyd}.}.
...
\begin{algorithm}
    ...
    let $l$ be the length of $m$: $l \gets \norm{m}$; \\
    $\delta \gets l - s$; \\
    \eIf{$\delta \geq 0$}{
        \Return{$m$}
    }{
        \eIf{$m[0] = \emph{\texttt{PAD}}_0$}{
            \eIf{$m[l-1] = \emph{\texttt{PAD}}_1$}{
                \Return{$\left( \emph{\texttt{PAD}}_2 \times \delta \right) \concat m$}
            }{
                \Return{$\left( \emph{\texttt{PAD}}_1 \times \delta \right) \concat m$}
            }
        }{
            \eIf{$m[l-1] = \emph{\texttt{PAD}}_0$}{
                ...
                \Return{$\left( \emph{\texttt{PAD}}_0 \times \delta \right) \concat m$}
            }
        }
    }
    \caption{Padding function $\mu$}
    \label{algo:padding}
\end{algorithm}
...
\begin{algorithm}
    ...
    seed $Rnd()$; \\
    set variables $pos \gets 0$ and $\bar{x} \gets \floor{ \frac{\norm{s}}{t} }$; \\
...
\begin{algorithm}
    \SetKwProg{throw}{throw}{}{}
    \KwIn{$Cr$, $\pi_H^{SK}$, $\mathcal{S}' := \mathcal{S} \setminus \varsigma_0$, an optional verification hash $V$}
    \KwOut{the recovered data or an error}
    from $Cr$, get the number $t$ of slices; \\
    \If{$\norm{\mathcal{S}'} \neq t - 1$}{
        \throw{not enough third-party slices}{}
    }
